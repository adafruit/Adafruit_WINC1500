...
public:
  Adafruit_WINC1500UDP();  // Constructor
	...
	// data)
  virtual uint8_t begin(uint16_t port, uint32_t multicastAddr);
  virtual uint8_t begin(uint16_t port) { begin(port, 0); }	// initialize, start listening on specified port. Returns 1 if successful, 0 if there are no sockets available to use
